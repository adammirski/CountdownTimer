...
class CountdownApp:
    ...
    def start_countdown(self):
        ...
        if self.total_milliseconds >= 0:
            ...
            # Toggle flash state every second (1000 milliseconds)
            if milliseconds % 500 == 0:
                self.flash = not self.flash
            
            # Update the label with the current time and flash effect
            self.label.config(text=time_format if self.flash else "", fg="red" if self.flash else "black")
            self.root.update()
